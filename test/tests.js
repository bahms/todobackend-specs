var chai = require('chai'),
    ...
    chaiAsPromised = require('chai-as-promised'); chai.use(chaiAsPromised); var url = process.env.URL || 'http://localhost:8000/todos'; 
...
//console.log(JSON.stringify(result));
    ...
    return assert(result,"header").to.contain.property('access-control-allow-origin','*');
  });
});
describe('Create Todo Item', function() {
  ...
  it('should receive a location hyperlink', function() {
    return assert(result, 'header').to.contain.property('location').that.match(/^http?:\/\/.+\/todos\/[\d]+$/);
  });
  it('should create the item', function() {
    var item = result.then(function (res) {
      return get(res.header['location']);
    });
    return assert(item, "body").to.contain.property('title').that.equals('Walk the dog');
  });
  after(function () {
    return del(url);
  });
...
describe('Update Todo Item', function() {
  ...
  it('should have completed set to true after PUT update', function() {
    ...
    return assert(result, "body").to.contain.property('completed').to.be.true;
  });
